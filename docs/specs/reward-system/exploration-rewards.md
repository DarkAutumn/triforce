...
11. **Terminal penalty + immediate termination creates short episodes that starve GAE of temporal context.** Observed in dungeon1-skip-opening training (multihead, 250k steps): every episode lasts only 3–24 steps before the agent walks south into the entry room (0x73), triggering both `PENALTY_WRONG_LOCATION` (-1.0) and `LeftPlayArea` termination simultaneously. The GAE computation correctly zero-bootstraps on termination (`mask = 1.0 - done`, so `delta = reward - V(s)`), meaning the -1.0 is fully attributed to the final step's advantage. However, with only 3–24 steps per episode, the backward GAE propagation (`γλ` chain) has very few steps to work with — the penalty signal decays to near-zero within ~5 steps at `γ=0.99, λ=0.95` (0.99×0.95)^5 ≈ 0.74, but there are so few *distinguishing* steps that the model cannot build a robust association between "walking south" and the terminal penalty. The model needs many rollouts of these short episodes to accumulate enough gradient signal, which explains why early training (< 500k steps) shows no avoidance behavior. This is exacerbated by the double punishment (Finding #10): the episode ends so abruptly that the agent never experiences recovery from a wrong room, only instant death.
...
11. **Allow recovery from wrong-room transitions instead of immediate termination** [Finding #11]. When the agent enters a wrong room, apply the penalty but don't terminate — instead, let the episode continue (the agent is now in a bad state and must navigate back or face further penalties). This gives GAE a longer trajectory with richer temporal structure: the agent experiences the consequence of a wrong move rather than just dying instantly. Longer episodes produce more gradient signal per rollout and allow the value function to learn that "near the wrong exit" states have low value, creating a natural avoidance gradient. This is especially important for dungeon scenarios where the entry room is one step south — the agent needs to learn spatial avoidance, not just "don't die."
